...
Within the `database` folder itself, run (on Linux or Mac)
```shell
...
### Test database not showing most up-to-date schema
In this case, delete `prod-schema.sql` to prompt the test logic to re-download the schema. The most up to date schema is not downloaded automatically, and only downloads when `prod-schema.sql` can't be found.
...
### database "tests_tmpl" already exists

In the case of getting this error when running pytest, try stopping the docker container from docker engine and reruning start_database_setup