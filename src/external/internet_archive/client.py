from aiohttp import ClientSession

from src.external.internet_archive.convert import convert_capture_to_archive_metadata
from src.external.internet_archive.models.archive_metadata import IAArchiveMetadata
from src.external.internet_archive.models.capture import IACapture


class InternetArchiveClient:

    def __init__(
        self,
        session: ClientSession
    ):
        self.session = session

    async def _get_url_snapshot(self, url: str) -> IACapture | None:
        params = {
            "url": url,
            "output": "json",
            "limit": "1",
            "gzip": "false",
            "filter": "statuscode:200",
            "fl": "timestamp,original,length,digest"
        }
        async with self.session.get(
            f"http://web.archive.org/cdx/search/cdx",
            params=params
        ) as response:
            raw_data = await response.json()
            if len(raw_data) == 0:
                return None
            fields = raw_data[0]
            values = raw_data[1]
            d = dict(zip(fields, values))
            return IACapture(**d)

    async def search_for_url_snapshot(self, url: str) -> IAArchiveMetadata | None:
        capture: IACapture | None = await self._get_url_snapshot(url)
        if capture is None:
            return None
        return convert_capture_to_archive_metadata(capture)
